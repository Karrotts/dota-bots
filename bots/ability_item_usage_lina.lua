...
function AbilityUsageThink()
	...
	--If Laguna blade has the highest priority then use it on the target
	...
	--if light strike array priority is greater than 0 then use it on target
	if ( castLSADesire > 0 )
	then
	...
	--if dragon slave has the priority is greater than 0 use it on target
	if ( castDSDesire > 0 )
	then
...
-- returns true if the NPC is able to use this ability on target
...
function ConsiderLightStrikeArray()
	...
	-- gets the hero and assigns it to npcBot
	local npcBot = GetBot();
